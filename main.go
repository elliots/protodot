...
func (pbs *pbstate) showInclusion(groupByPackages bool, leaveRootPackageUnwrapped bool) {
	...
	for from, tos := range pbs.inclusions {
		for to, _ := range tos {
			...
			fromName := bits[0]
			var event bool
			if strings.HasPrefix(fromName, "ev-") {
				fromName = strings.TrimPrefix(fromName, "ev-")
				event = true
			}
			args := Relationship{
				From:   fromName,
			...
			if event {
				tmplName = toTemplateName[typenameEvent]
				args.Field = "header"
			}
...
func (pbs *pbstate) handleMessageBody(msg *proto.Message) {
	...
	t := newTable(message, info.fullname, info.unique, getStyle(msg))
...
func getStyle(msg *proto.Message) string {
	for _, e := range msg.Elements {
		if o, ok := e.(*proto.Option); ok && (o.Name == "(es.v2.document_type)") {
			return "document"
		}
		if o, ok := e.(*proto.Option); ok && (o.Name == "(es.v2.event_type)") {
			return "event"
		}
		if o, ok := e.(*proto.Option); ok && (o.Name == "(es.v2.command_type)") {
			return "command"
		}
	}
	return "normal"
...
func (pbs *pbstate) handleEventTypeResolution(msg *proto.Message) {

	fullname := getFullName(msg)

	for _, e := range msg.Elements {
		if o, ok := e.(*proto.Option); ok && (o.Name == "(es.v2.must_output)" || o.Name == "(es.v2.can_output)") {
			pbs.resolveType(fullname, OriginalName(o.Constant.Source))
		}
	}

}

func (pbs *pbstate) handleEvents(msg *proto.Message) {
	...
	for _, e := range msg.Elements {
		if o, ok := e.(*proto.Option); ok && (o.Name == "(es.v2.must_output)" || o.Name == "(es.v2.can_output)") {
			...
			if inf := pbs.getResolution(full, OriginalName(o.Constant.Source)); inf != nil {
				pbs.encounteredType("ev-"+info.unique, "", inf.unique)
			} else {
				log.Printf("failed to resolve: %s", o.Constant.Source)
				pbs.recordMissingInclusion("ev-"+info.unique, "", OriginalName(o.Constant.Source))
			}
...
func process(pbs *pbstate, name string, selection string) bool {
	...
	proto.Walk(definition,
		...
		proto.WithMessage(pbs.handleEventTypeResolution),
	...
	proto.Walk(definition,
		proto.WithMessage(pbs.handleEvents))
